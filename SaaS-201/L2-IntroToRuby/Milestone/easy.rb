...
# ARGV is an array that Ruby defines
for us,
...
# `domain`
contains the domain name we have to look up.
...
dns_filter_list = []
dns_filter.each {
  | x | dns_filter_list.push(x.split(", "))
}

# Creating the List each DNS
for Hash
record_type_list = []
source_list = []
destination_list = []
...
# To complete the assignment, implement `parse_dns`
and `resolve`.
# Remember to implement them above this line since in Ruby
# you can invoke a
function only after it is defined.
